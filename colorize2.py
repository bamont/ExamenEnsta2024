...
def produit_matrice_parallele(A,b):
    reste_i= A.shape[0] % nbp
    i_loc = A.shape[0]//nbp + (1 if rank<reste_i else 0)

    # List_i[rank] contient à la fin la valeur de i à laquelle doit s'arrêter le processus rank
    List_i=comm.allgather(i_loc)
    for i in range(1,nbp):
        List_i[i]+=List_i[i-1]

    # Chaque processus effectue une partie du produit matriciel
    c=A[List_i[rank]-i_loc:List_i[rank],:].dot(b)

    # On rassemble les parties calculées
    c=comm.allgather(c)
...
def minimize( A : sparse.csr_matrix, b : np.array, x0 : np.array, niters : int, epsilon : float):
    ...
    """
    # r = b-A.dot(x0)
    r= b-produit_matrice_parallele(A,x0)
    nrm_r0 = linalg.norm(r)
    #gc = A.transpose().dot(r)
    gc=produit_matrice_parallele(A.transpose(),r)
    x = np.copy(x0)
    p = np.copy(gc)
    # cp = A.dot(p)
    cp = produit_matrice_parallele(A,p)
