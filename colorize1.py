...
if __name__ == '__main__':
    ...
    (ny,nx,_)=values_gray.shape

    reste_ny= ny % nbp
    ny_loc = ny//nbp + (1 if rank<reste_ny else 0)

    # List_ny[rank] contient à la fin la valeur de ny à laquelle doit s'arrêter le processus rank
    List_ny=comm.allgather(ny_loc)
    for i in range(1,nbp):
        List_ny[i]+=List_ny[i-1]

    # On divise en plusieurs tranches 
    values_gray = np.array(im_gray)[List_ny[rank]-ny_loc:List_ny[rank]-1]
    ...
    val_ycbcr = np.array(im_ycbcr)[List_ny[rank]-ny_loc:List_ny[rank]-1]
    ...
    # On regroupe les différentes images vers le processus 0 qui s'occupe de la sauvegarde
    new_image_array=comm.gather(new_image_array,root=0)

    if rank==0:
        new_image_array=np.concatenate(new_image_array,axis=0)
        image=Image.fromarray(new_image_array, mode='YCbCr')
        image.convert('RGB').save(output, 'PNG')
    fin=time.time()
    if rank==0:
        print("Le temps total d'exéction est de :",fin-début)